            ...
            <form id="formOS" action="./php/insert_train.php" method="post">
                <fieldset>
                    ...
                    <div class="form-group">
                        <label for="itinerario_rota">Itinerário:*</label>
                        <select id="itinerario_rota" name="itinerario_rota" required>
                            <option value="">Selecione...</option>
                            <?php
                            include("listar_itinerario.php");
                            foreach ($itinerarios as $linha) {
                                echo  "<option value='" . $linha['id_itinerario'] . "'>" . $linha['origem_itinerario'] . "</option>";
                            } ?>
                        </select>
                    </div>

                    <div class="form-group">
                        <label for="modelo-trem">Caminho da rota:*</label>
                        <input id="modelo-trem" name="modelo-trem" rows="4" required></textarea>
                    </div>

                    <button type="submit" value="Adicionar Nova Rota">Adicionar Nova Rota</button>
            </form>
        </div>
    </section>
